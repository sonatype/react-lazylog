...
import { LazyLogLine } from "./LazyLogLine/LazyLogLine";
import { LazyLogLoading } from "./LazyLogLoading/LazyLogLoading";
import { LazyLogSearchBar } from "./LazyLogSearchBar/LazyLogSearchBar";
import request from "./request";
import stream from "./stream";
import websocket from "./websocket";
import { searchLines } from "./search";
import { lazyLog, searchMatch } from "./LazyLog.module.css";
...
export class LazyLog extends Component {
  static propTypes = {
    /**
    ...
    /**
     * Pass In line Props as a data object
     */
    lineData: object,
  };
  ...
  static defaultProps = {
    ...
    loadingComponent: LazyLogLoading,
    lineClassName: "",
  ...
  static getDerivedStateFromProps(
    ...
    const shouldUpdate =
      ...
      (nextMessageData && nextMessageData !== previousMessageData);
  ...
  request() {
    const { messageData, url } = this.props;

    this.endRequest();

    if (messageData) {
      const encodedLog = encode(
        messageData.map(({ content }) => content).join("\n")
      );
    ...
    if (url) {
      throw "Need to convert convertBufferToLines to messageData";
  ...
  toggleOpenTopic = (topic) => {
    const { closedTopics: previousClosed } = this.state;

    const closedTopics = previousClosed.includes(topic)
      ? previousClosed.filter((t) => t !== topic)
      : [...previousClosed, topic];

    this.setState({
      closedTopics,
      hiddenTopics: this.props.parser.hiddenTopics(closedTopics),
    });
  };
  ...
  lineInfo = (index) => {
    const { lines, isFilteringLinesWithMatches, filteredLines } = this.state;
    const linesToRender = isFilteringLinesWithMatches ? filteredLines : lines;

    const lineData = linesToRender.get(index);

    const isOpen = lineData
      ? !this.state.hiddenTopics.has(lineData.topic) ||
        (lineData.isHeader && this.state.closedTopics.includes(lineData.topic))
      : true;
  ...
  renderRow = ({ key, index, style }) => {
    ...
    const { lineData, isOpen } = this.lineInfo(index);
    ...
    return (
      <LazyLogLine
        ...
        data={[{ text: lineData.content }]}
        lineData={lineData}
        toggleOpenTopic={this.toggleOpenTopic}
        isOpen={isOpen}
        isExpanded={!this.state.closedTopics.includes(lineData.topic)}
      ...
      <Fragment>
        ...
        >
          {({ height, width }) => {
            return (
              <VirtualList
                className={`react-lazylog ${lazyLog} log__list`}
                rowCount={
                ...
                rowHeight={({ index }) =>
                  this.lineInfo(index).isOpen ? this.props.rowHeight : 0
                }
